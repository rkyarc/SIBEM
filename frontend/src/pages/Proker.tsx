...
const Proker = () => {
  ...
  const fetchProkers = async (silent = false) => {
    ...
    try {
      ...
      const response = await axios.get("https://sibem-git-fix-deploy-rkyarcs-projects.vercel.app/api/proker", {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEditMode && editId !== null) {
        await axios.put(
          `https://sibem-git-fix-deploy-rkyarcs-projects.vercel.app/api/proker/${editId}`,
          formData,
          {
            headers: { Authorization: `Bearer ${token}` },
          },
        ...
        await axios.post("https://sibem-git-fix-deploy-rkyarcs-projects.vercel.app/api/proker", formData, {
          headers: { Authorization: `Bearer ${token}` },
        });
      }
  ...
  const handleStatusChange = async (kegiatan: ProkerData, newStatus: string) => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.put(`https://sibem-git-fix-deploy-rkyarcs-projects.vercel.app/api/proker/${kegiatan.id}`, {
        ...kegiatan,
        status: newStatus
      }, {
        headers: { Authorization: `Bearer ${token}` },
  ...
  const confirmDelete = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.delete(`https://sibem-git-fix-deploy-rkyarcs-projects.vercel.app/api/proker/${targetId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
